...
use crate::{
    constants::ZSTD_LEVEL, error::CarbonadoError, filepack_manifest::MAX_SEGMENT_MAIN_LEN,
};
...
/// Stream-compress `input` into `output` at [`crate::constants::ZSTD_LEVEL`] (20).
/// Returns compressed bytes written.
///
/// Frame flags match Lean `Carbonado.Compress` where they can: checksum off, no dictionary.
/// Streaming `copy_encode` leaves content size unknown (descriptor differs from Lean AOT
/// one-shot `ZSTD_compress` frames). That is an honest frame-shape difference, not a bug.
